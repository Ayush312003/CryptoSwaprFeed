...
const CryptoDetails = () => {
  ...
  const genericStats = [
    ...
    { title: 'Total Supply', value: `$ ${parseFloat(cryptoDetails?.supply?.total) && millify(parseFloat(cryptoDetails?.supply?.total))}`, icon: <ExclamationCircleOutlined /> },
        ...
        <Col className="coin-value-statistics">
          ...
          {stats.map(({ icon, title, value }) => (
            <Col className="coin-stats">
              ...
              <Text style={{ color: 'orange' }} className="stats">{value}</Text>
            </Col>
          ))}
        </Col>
        ...
        <Col className="coin-links">
          ...
          {cryptoDetails.links?.map((link) => (
            <Row className="coin-link" key={link.name}>
              <Title level={5} style={{ color: 'orange' }} className="link-name">{link.type}</Title>
